...
var applicationUrl = "http://" + domain + ":" + PORT + "/api";
...
const authenticateToken = (req, res, next) => {
  ...
  jwt.verify(token, JWT_SECRET, async (err, user) => {
    ...
    const insertIntoBlacklistSql =
      "INSERT INTO blacklisted_email (email) VALUES (?)";
...
app.post("/signup/", async (req, res, next) => {
  ...
  const blacklistedEmailCheckSql =
    "SELECT email FROM blacklisted_email WHERE email = ?";
...
app.post("/signin/", async (req, res, next) => {
  ...
  try {
    ...
    if (!data.rows.length) {
      res
        .status(400)
        .json({ error: "Invalid username/password or user does not exist!" });
      return;
    }
...
app.get("/myposts", authenticateToken, async (req, res, next) => {
  const sql =
    "SELECT * FROM post WHERE authorId = ? ORDER BY date_created DESC";
  const params = [req.user.id];
...
app.get("/posts/upvote/:id", authenticateToken, async (req, res, next) => {
  ...
  const upvoteUserSql =
    "UPDATE user SET life_points = (life_points + 1) WHERE id = ?";
